# `torque stack` verify (Kubernetes-only health gates)

`torque stack` can optionally run a post-apply **verify** phase per release. Verification is Kubernetes-only (no Prometheus): it uses workload readiness signals plus recent Warning events tied to the release manifest inventory.
...
defaults:
  ...
  verify:
    ...
    # allowReasons: ["TorqueVerifyDemo"]   # optional allowlist (case-insensitive)
    # requireConditions:
    #   - group: example.com
    #     kind: Widget
    #     type: Ready
...
- Event watermarking: when the sqlite run store is enabled, torque records the Events list `resourceVersion` per namespace after successful verifies and uses it as a best-effort watermark to ignore old Warning events on subsequent runs (falls back to the time window when RVs are missing/unparseable).
