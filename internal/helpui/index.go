...
func BuildIndex(root *cobra.Command, includeHidden bool) Index {
	...
	if md := strings.TrimSpace(torquedocs.ProofGraphMD); md != "" {
		entries = append(entries, Entry{
			...
			Subtitle: "Signed release graph, gate, attest, agent policy, score, and flight recorder evidence",
			Content:  md,
			Tags:     []string{"doc", "proof", "graph", "signature", "verify", "diff", "gate", "attest", "agent", "score", "flight", "evidence", "release"},
		})
	}
