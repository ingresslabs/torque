...
func wrapUpgradeOnlyNoDeployedReleaseErr(releaseName, namespace string, err error) error {
	...
	return fmt.Errorf(
		"helm upgrade: %w; release %q is not deployed in namespace %q (omit --upgrade to allow install fallback, or pick an existing release name from `torque list --namespace %s`)",
		err,
		releaseName,
		namespace,
		namespace,
