...
// deploy_plan.go contains the deploy plan logic (helm plan), rendering manifests, producing HTML diffs, and teeing the plan into files.
package main
...
func newDeployPlanCommand(namespace *string, kubeconfig *string, kubeContext *string, helpSection string) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long:  "Render the chart, diff it against live cluster resources, and summarize the net creates/updates/deletes before applying.",
		Args:  cobra.NoArgs,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			case "html":
				...
				if strings.TrimSpace(path) == "" {
					...
					path = fmt.Sprintf("helmer-deploy-plan-%s-%s.html", slug, planResult.GeneratedAt.Format("20060102-150405"))
				}
	...
	cmd.Flags().StringVar(&outputPath, "output", "", "Write the rendered plan to this path (HTML defaults to ./helmer-deploy-plan-<release>-<timestamp>.html)")
...
func defaultDeployVisualizeDataOutputPath(release string, generatedAt time.Time, ext string) string {
	...
	return fmt.Sprintf("helmer-deploy-visualize-%s-%s.%s", slug, stamp.Format("20060102-150405"), ext)
}
...
func defaultDeployVisualizeOutputPath(release string, generatedAt time.Time) string {
	...
	return fmt.Sprintf("helmer-deploy-visualize-%s-%s.html", slug, generatedAt.Format("20060102-150405"))
}

func buildInstallCommand(opts deployPlanOptions) string {
	parts := []string{"helmer", "plan"}
	if opts.Chart != "" {
