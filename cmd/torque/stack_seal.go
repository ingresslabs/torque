// File: cmd/torque/stack_seal.go
// Brief: `torque stack seal` command wiring.
...
func newStackSealCommand(rootDir, profile *string, clusters *[]string, inferDeps *bool, inferConfigRefs *bool, tags *[]string, fromPaths *[]string, releases *[]string, gitRange *string, gitIncludeDeps *bool, gitIncludeDependents *bool, includeDeps *bool, includeDependents *bool, allowMissingDeps *bool) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if out == "" {
				out = filepath.Join(*rootDir, ".torque", "stack", "sealed", runID)
			}
			...
			fmt.Fprintf(cmd.ErrOrStderr(), "torque stack seal: wrote %s (planHash=%s)\n", out, planHash)
			return nil
		},
	}
	cmd.Flags().StringVar(&outDir, "out", "", "Output directory (defaults to --root/.torque/stack/sealed/<runId>)")
