// File: cmd/torque/init_from_cluster.go
// Brief: "torque init from-cluster" onboarding flow.
...
import (
	...
	"github.com/ingresslabs/torque/internal/clusteradopt"
...
func printInitFromClusterSummary(out interface{ Write([]byte) (int, error) }, s initFromClusterSummary) {
	...
	fmt.Fprintf(out, "torque init from-cluster %s %s\n", mode, target)
	if s.KubeNote != "" {
	...
	fmt.Fprintln(out, "next: torque stack plan --config stack.yaml")
	fmt.Fprintln(out, "next: torque stack verify --config stack.yaml")
}
