...
// File: cmd/torque/build_fixtures_integration_test.go
...
func runBuildFixture(t *testing.T, contextDir string, extraArgs, extraEnv []string) {
	...
	tag := fmt.Sprintf("torque.local/e2e/%s:%d", filepath.Base(contextDir), time.Now().UnixNano())
	...
	if err := cmd.Run(); err != nil {
		t.Fatalf("torque build %s failed: %v\n%s", contextDir, err, buf.String())
	}
	output := buf.Bytes()
	if containsComposeMode(extraArgs) {
		if !bytes.Contains(output, []byte(": torque")) {
