// File: cmd/torque/deploy_plan_html_test.go
...
func TestRenderDeployPlanHTML(t *testing.T) {
	result := &deployPlanResult{
		...
		InstallCmd:  "torque apply --chart ./chart --release demo",
...
func TestPlanHTMLEmbedsJSON(t *testing.T) {
	...
	if !strings.Contains(html, `id="torquePlanData"`) {
...
func TestRenderDeployVisualizeHTML(t *testing.T) {
	...
	if !strings.Contains(html, "torque Apply Plan Visualize") {
...
func TestRenderDeployVisualizeHTMLWithExplainDiffFeature(t *testing.T) {
	...
	if !strings.Contains(html, `id="torqueVizFeatures"`) {
