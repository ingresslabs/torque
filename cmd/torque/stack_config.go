...
func resolveStackCommandConfig(cmd *cobra.Command, common stackCommandCommon) (stackCommandConfig, error) {
	...
	if cmd != nil && !flagChanged(cmd, "root") {
		if v := strings.TrimSpace(os.Getenv("TORQUE_STACK_ROOT")); v != "" {
			root = v
		}
	}
	if root == "" {
	...
	if cmd != nil && !flagChanged(cmd, "profile") {
		if v := strings.TrimSpace(os.Getenv("TORQUE_STACK_PROFILE")); v != "" {
			profile = v
		}
	}
	...
	if strings.TrimSpace(cfg.Selector.GitRange) == "" && (cfg.Selector.GitIncludeDeps || cfg.Selector.GitIncludeDependents) {
		return stackCommandConfig{}, fmt.Errorf("invalid selector: gitIncludeDeps/gitIncludeDependents requires selector.gitRange (set cli.selector.gitRange or TORQUE_STACK_GIT_RANGE)")
	}
...
func resolveInferDefaults(cmd *cobra.Command, common stackCommandCommon, cfg stack.StackCLIResolved) (inferDeps bool, inferConfigRefs bool, err error) {
	...
	} else if v, ok, err := envBool("TORQUE_STACK_INFER_DEPS"); err != nil {
	...
	} else if v, ok, err := envBool("TORQUE_STACK_INFER_CONFIG_REFS"); err != nil {
		return false, false, err
	} else if ok {
		inferConfigRefs = v
	}
...
func resolveOutputDefault(cmd *cobra.Command, common stackCommandCommon, cfg stack.StackCLIResolved) (string, error) {
	...
	if v := strings.TrimSpace(os.Getenv("TORQUE_STACK_OUTPUT")); v != "" {
...
func resolveSelectorDefaults(cmd *cobra.Command, common stackCommandCommon, cfg stack.StackCLIResolved) ([]string, stack.Selector, []string, error) {
	...
	clusters := resolveStringSliceDefault(cmd, "cluster", splitCSV(derefStringSlice(common.clusters)), "TORQUE_STACK_CLUSTER", cfg.Clusters)
	selector := cfg.Selector

	selector.Tags = resolveStringSliceDefault(cmd, "tag", splitCSV(derefStringSlice(common.tags)), "TORQUE_STACK_TAG", cfg.Selector.Tags)
	selector.FromPaths = resolveStringSliceDefault(cmd, "from-path", splitCSV(derefStringSlice(common.fromPaths)), "TORQUE_STACK_FROM_PATH", cfg.Selector.FromPaths)
	selector.Releases = resolveStringSliceDefault(cmd, "release", splitCSV(derefStringSlice(common.releases)), "TORQUE_STACK_RELEASE", cfg.Selector.Releases)

	if flagChanged(cmd, "git-range") {
		selector.GitRange = strings.TrimSpace(derefString(common.gitRange))
	} else if v := strings.TrimSpace(os.Getenv("TORQUE_STACK_GIT_RANGE")); v != "" {
		if strings.TrimSpace(cfg.Selector.GitRange) != "" {
			warnings = append(warnings, "both stack.yaml cli.selector.gitRange and TORQUE_STACK_GIT_RANGE are set; using TORQUE_STACK_GIT_RANGE")
		}
		selector.GitRange = v
	} else {
	...
	selector.GitIncludeDeps = resolveBoolDefault(cmd, "git-include-deps", common.gitIncludeDeps, "TORQUE_STACK_GIT_INCLUDE_DEPS", cfg.Selector.GitIncludeDeps)
	selector.GitIncludeDependents = resolveBoolDefault(cmd, "git-include-dependents", common.gitIncludeDependents, "TORQUE_STACK_GIT_INCLUDE_DEPENDENTS", cfg.Selector.GitIncludeDependents)
	selector.IncludeDeps = resolveBoolDefault(cmd, "include-deps", common.includeDeps, "TORQUE_STACK_INCLUDE_DEPS", cfg.Selector.IncludeDeps)
	selector.IncludeDependents = resolveBoolDefault(cmd, "include-dependents", common.includeDependents, "TORQUE_STACK_INCLUDE_DEPENDENTS", cfg.Selector.IncludeDependents)
	selector.AllowMissingDeps = resolveBoolDefault(cmd, "allow-missing-deps", common.allowMissingDeps, "TORQUE_STACK_ALLOW_MISSING_DEPS", cfg.Selector.AllowMissingDeps)
...
func resolveRunDefaults(cmd *cobra.Command, cfg stack.StackCLIResolved) (apply stackRunDefaultOverrides, del stackRunDefaultOverrides, resume stackResumeDefaultOverrides, err error) {
	...
	if !flagChanged(cmd, "dry-run") {
		if v, ok, err := envBool("TORQUE_STACK_APPLY_DRY_RUN"); err != nil {
	...
	if !flagChanged(cmd, "diff") {
		if v, ok, err := envBool("TORQUE_STACK_APPLY_DIFF"); err != nil {
			return stackRunDefaultOverrides{}, stackRunDefaultOverrides{}, stackResumeDefaultOverrides{}, err
		} else if ok {
			apply.Diff = &v
	...
	if !flagChanged(cmd, "fail-fast") && !flagChanged(cmd, "continue-on-error") {
		if v, ok, err := envBool("TORQUE_STACK_APPLY_FAIL_FAST"); err != nil {
			return stackRunDefaultOverrides{}, stackRunDefaultOverrides{}, stackResumeDefaultOverrides{}, err
		} else if ok {
			apply.FailFast = &v
		...
		if v, ok, err := envBool("TORQUE_STACK_DELETE_FAIL_FAST"); err != nil {
			return stackRunDefaultOverrides{}, stackRunDefaultOverrides{}, stackResumeDefaultOverrides{}, err
		} else if ok {
			del.FailFast = &v
	...
	if !flagChanged(cmd, "retry") {
		if v, ok, err := envInt("TORQUE_STACK_APPLY_RETRY"); err != nil {
			return stackRunDefaultOverrides{}, stackRunDefaultOverrides{}, stackResumeDefaultOverrides{}, err
		} else if ok {
			apply.Retry = &v
		...
		if v, ok, err := envInt("TORQUE_STACK_DELETE_RETRY"); err != nil {
			return stackRunDefaultOverrides{}, stackRunDefaultOverrides{}, stackResumeDefaultOverrides{}, err
		} else if ok {
			del.Retry = &v
	...
	if !flagChanged(cmd, "lock") {
		if v, ok, err := envBool("TORQUE_STACK_APPLY_LOCK"); err != nil {
			return stackRunDefaultOverrides{}, stackRunDefaultOverrides{}, stackResumeDefaultOverrides{}, err
		} else if ok {
			apply.Lock = &v
		...
		if v, ok, err := envBool("TORQUE_STACK_DELETE_LOCK"); err != nil {
			return stackRunDefaultOverrides{}, stackRunDefaultOverrides{}, stackResumeDefaultOverrides{}, err
		} else if ok {
			del.Lock = &v
	...
	if !flagChanged(cmd, "takeover") {
		if v, ok, err := envBool("TORQUE_STACK_APPLY_TAKEOVER"); err != nil {
			return stackRunDefaultOverrides{}, stackRunDefaultOverrides{}, stackResumeDefaultOverrides{}, err
		} else if ok {
			apply.Takeover = &v
		...
		if v, ok, err := envBool("TORQUE_STACK_DELETE_TAKEOVER"); err != nil {
			return stackRunDefaultOverrides{}, stackRunDefaultOverrides{}, stackResumeDefaultOverrides{}, err
		} else if ok {
			del.Takeover = &v
	...
	if !flagChanged(cmd, "lock-ttl") {
		if v, ok, err := envDuration("TORQUE_STACK_APPLY_LOCK_TTL"); err != nil {
			return stackRunDefaultOverrides{}, stackRunDefaultOverrides{}, stackResumeDefaultOverrides{}, err
		} else if ok {
			apply.LockTTL = &v
		...
		if v, ok, err := envDuration("TORQUE_STACK_DELETE_LOCK_TTL"); err != nil {
			return stackRunDefaultOverrides{}, stackRunDefaultOverrides{}, stackResumeDefaultOverrides{}, err
		} else if ok {
			del.LockTTL = &v
	...
	if !flagChanged(cmd, "lock-owner") {
		if v := strings.TrimSpace(os.Getenv("TORQUE_STACK_APPLY_LOCK_OWNER")); v != "" {
			apply.LockOwner = &v
		} else if cfg.ApplyLockOwner != nil && strings.TrimSpace(*cfg.ApplyLockOwner) != "" {
			apply.LockOwner = cfg.ApplyLockOwner
		}

		if v := strings.TrimSpace(os.Getenv("TORQUE_STACK_DELETE_LOCK_OWNER")); v != "" {
			del.LockOwner = &v
	...
	if !flagChanged(cmd, "delete-confirm-threshold") {
		if v, ok, err := envInt("TORQUE_STACK_DELETE_CONFIRM_THRESHOLD"); err != nil {
			return stackRunDefaultOverrides{}, stackRunDefaultOverrides{}, stackResumeDefaultOverrides{}, err
		} else if ok {
	...
	if !flagChanged(cmd, "allow-drift") {
		if v, ok, err := envBool("TORQUE_STACK_RESUME_ALLOW_DRIFT"); err != nil {
			return stackRunDefaultOverrides{}, stackRunDefaultOverrides{}, stackResumeDefaultOverrides{}, err
		} else if ok {
			resume.AllowDrift = &v
	...
	if !flagChanged(cmd, "rerun-failed") {
		if v, ok, err := envBool("TORQUE_STACK_RESUME_RERUN_FAILED"); err != nil {
			return stackRunDefaultOverrides{}, stackRunDefaultOverrides{}, stackResumeDefaultOverrides{}, err
		} else if ok {
