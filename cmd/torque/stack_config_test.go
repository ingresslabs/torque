...
func TestStackPlan_WarnsWhenEnvGitRangeOverridesStackYAML(t *testing.T) {
	...
	t.Setenv("TORQUE_STACK_ROOT", root)
	t.Setenv("TORQUE_STACK_GIT_RANGE", "HEAD~1..HEAD")
	...
	if !strings.Contains(strings.ToLower(errOut.String()), "both stack.yaml cli.selector.gitrange and torque_stack_git_range are set") {
...
func writeGitRangedStack(t *testing.T) string {
	...
	_ = run(t, root, "git", "config", "user.email", "torque@example.invalid")
	_ = run(t, root, "git", "config", "user.name", "torque")
