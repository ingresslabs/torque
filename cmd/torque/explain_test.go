...
func TestExplainCommandPrintsCaptureSummary(t *testing.T) {
	...
	for _, want := range []string{
		...
		"Outcome: failed",
		"upgrade failed",
		"torque revert --release api -n prod",
	} {
...
func TestExplainCommandJSONFormat(t *testing.T) {
	...
	if got := out.String(); !strings.Contains(got, `"sessions"`) || !strings.Contains(got, `"command": "torque logs"`) {
...
func TestExplainCommandMarkdownFormat(t *testing.T) {
	...
	got := out.String()
	for _, want := range []string{"# torque explain", "Primary cause", "rollout_timeout", "torque revert --release api -n prod"} {
		if !strings.Contains(got, want) {
