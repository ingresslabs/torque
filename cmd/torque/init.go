...
func newInitCommand(kubeconfig *string, kubeContext *string, profile *string) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "init [path]",
		Short: "Initialize a repo with torque defaults",
		Long: strings.TrimSpace(`
Creates a repo-local .torque.yaml with sane defaults and prints a short
getting-started checklist.`),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !opts.merge || !existing.Exists {
				header := []byte("# torque repo config\n")
			...
			if opts.gitignore || opts.gitignoreConfig {
				...
				if opts.gitignoreConfig {
					entries = append(entries, ".torque.yaml")
				}
				if len(entries) > 0 {
	...
	cmd.Flags().BoolVar(&opts.force, "force", false, "Overwrite an existing .torque.yaml")
	cmd.Flags().BoolVar(&opts.merge, "merge", false, "Merge defaults into an existing .torque.yaml")
	cmd.Flags().BoolVar(&opts.dryRun, "dry-run", false, "Print the .torque.yaml that would be written")
	...
	cmd.Flags().BoolVar(&opts.gitignoreConfig, "gitignore-config", false, "Also add .torque.yaml to .gitignore")
	cmd.Flags().BoolVar(&opts.showDiff, "show-diff", false, "Show a diff when merging into an existing .torque.yaml")
	...
	cmd.Flags().StringVar(&opts.secretsFile, "secrets-file", opts.secretsFile, "Local secrets file path to reference in .torque.yaml")
...
func runInitWizard(cmd *cobra.Command, opts *initOptions, project projectLayout, profileValue *string) error {
	...
	gitignoreCfgChoice, err := promptBool(reader, out, "Add .torque.yaml to .gitignore", opts.gitignoreConfig)
	if err != nil {
		return err
	}
...
func updateGitignore(repoRoot string, entries []string, dryRun bool) (gitignoreResult, error) {
	...
	if _, ok := existing["# torque"]; !ok {
		if _, err := f.WriteString("# torque\n"); err != nil {
			return res, err
		}
	}
...
func buildNextSteps(project projectLayout, layoutEnabled bool, namespace string) []string {
	...
	if project.ChartPath != "" {
		steps = append(steps, fmt.Sprintf("torque apply plan --chart %s --release demo -n %s%s", project.ChartPath, namespace, valuesArg))
		steps = append(steps, fmt.Sprintf("torque apply --chart %s --release demo -n %s%s --ui", project.ChartPath, namespace, valuesArg))
		if project.ValuesDevPath != "" && project.ValuesProdPath != "" && project.ValuesDevPath != project.ValuesProdPath {
			steps = append(steps, fmt.Sprintf("torque apply plan --chart %s --release demo -n %s -f %s", project.ChartPath, namespace, project.ValuesProdPath))
		}
	...
	if project.StackPath != "" {
		steps = append(steps, fmt.Sprintf("torque stack plan --config %s", project.StackPath))
	}
	if len(steps) == 0 {
		steps = append(steps, fmt.Sprintf("torque apply plan --chart ./chart --release demo -n %s", namespace))
	}
	steps = append(steps, "torque help --ui")
	return steps
}
...
func buildInitNotes(project projectLayout, sandboxNote string, templateSource string) []string {
	...
	if project.HelmfilePath != "" {
		notes = append(notes, fmt.Sprintf("Helmfile detected at %s (torque init does not import Helmfile automatically).", project.HelmfilePath))
	}
	if project.KustomizePath != "" {
		notes = append(notes, fmt.Sprintf("Kustomize detected at %s. torque focuses on Helm workflows.", project.KustomizePath))
	}
	if project.DockerfilePath != "" {
		notes = append(notes, fmt.Sprintf("Dockerfile detected at %s. Try torque build --context . --tag <image>.", project.DockerfilePath))
	}
	if sandboxNote != "" {
...
func shouldSkipInitDir(name string) bool {
	switch name {
	case ".git", ".github", ".idea", ".vscode", "node_modules", "vendor", "dist", "bin", "testdata", "tmp", ".torque":
		return true
	default:
		return false
	}
...
func printInitSummary(w io.Writer, summary initSummary) {
	...
	case "merged":
		fmt.Fprintf(w, "Merged torque config at %s\n", summary.ConfigPath)
	case "overwritten":
		fmt.Fprintf(w, "Overwrote torque config at %s\n", summary.ConfigPath)
	default:
		fmt.Fprintf(w, "Initialized torque config at %s\n", summary.ConfigPath)
	}
	if summary.RepoRoot != "" {
