// File: cmd/torque/stack_export.go
// Brief: `torque stack export` command wiring.
...
func newStackExportCommand(rootDir *string) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if out == "" {
				out = filepath.Join(*rootDir, ".torque", "stack", "exports", id+".tgz")
			}
			wrote, err := stack.ExportRunBundle(cmd.Context(), *rootDir, id, out)
			if err != nil {
				return err
			}
			fmt.Fprintf(cmd.ErrOrStderr(), "torque stack export: wrote %s\n", wrote)
			return nil
		},
	}
	cmd.Flags().StringVar(&runID, "run-id", "", "Run ID to export (defaults to most recent)")
	cmd.Flags().StringVar(&outPath, "out", "", "Output bundle path (defaults to --root/.torque/stack/exports/<runId>.tgz)")
	return cmd
}
