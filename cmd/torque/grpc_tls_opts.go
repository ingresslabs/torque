...
func remoteTLSConfig(cmd *cobra.Command) (enabled bool, caPath string, serverName string, insecureSkipVerify bool, clientCert string, clientKey string, _ error) {
	...
	if v, ok, err := envBool("TORQUE_REMOTE_TLS"); err != nil {
		return false, "", "", false, "", "", err
	} else if ok {
		enabled = enabled || v
	}
	if caPath == "" {
		caPath = strings.TrimSpace(os.Getenv("TORQUE_REMOTE_TLS_CA"))
	}
	if serverName == "" {
		serverName = strings.TrimSpace(os.Getenv("TORQUE_REMOTE_TLS_SERVER_NAME"))
	}
	if v, ok, err := envBool("TORQUE_REMOTE_TLS_INSECURE_SKIP_VERIFY"); err != nil {
		return false, "", "", false, "", "", err
	} else if ok {
		insecureSkipVerify = insecureSkipVerify || v
	}
	if clientCert == "" {
		clientCert = strings.TrimSpace(os.Getenv("TORQUE_REMOTE_TLS_CLIENT_CERT"))
	}
	if clientKey == "" {
		clientKey = strings.TrimSpace(os.Getenv("TORQUE_REMOTE_TLS_CLIENT_KEY"))
	}
