...
`
	...
	written := filepath.Join(dir, ".torque.yaml")
	raw, err := os.ReadFile(written)
	if err != nil {
		t.Fatalf("read .torque.yaml: %v", err)
	}

	var cfg appconfig.Config
	if err := yaml.Unmarshal(raw, &cfg); err != nil {
		t.Fatalf("unmarshal .torque.yaml: %v", err)
	}
...
func TestInitDryRunDoesNotWrite(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(dir, ".torque.yaml")); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("expected no .torque.yaml to be written, got err=%v", err)
	}
...
`
	if err := os.WriteFile(filepath.Join(dir, ".torque.yaml"), []byte(existing), 0o600); err != nil {
		t.Fatalf("write .torque.yaml: %v", err)
	}
	...
	raw, err := os.ReadFile(filepath.Join(dir, ".torque.yaml"))
	if err != nil {
		t.Fatalf("read .torque.yaml: %v", err)
	}
	var cfg appconfig.Config
...
func TestInitTemplateFile(t *testing.T) {
	...
	if !strings.Contains(payload.ConfigYAML, "cacheDir: .torque/cache") {
...
func TestInitApplyPlanWritesFiles(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(dir, ".torque.yaml")); err != nil {
		t.Fatalf("expected .torque.yaml to be written, got err=%v", err)
	}
