...
Examples:
  ...
  verifier init namespace -n default --context my-context --write verify.yaml
  verifier init manifest --manifest ./rendered.yaml --write verify.yaml
`),
		Args: cobra.NoArgs,
	...
	// Shared flags for scripting convenience (only used when calling `verifier init` without a subcommand).
...
func newVerifyInitChartCommand() *cobra.Command {
  ...
  verifier init chart --chart ./chart --release foo -n default --use-cluster --context my-context --write verify.yaml
`),
		Args: cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := cfgpkg.BuildFromParams(cfgpkg.Params{
