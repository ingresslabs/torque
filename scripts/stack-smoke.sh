...
echo ">> apply"
./bin/torque --kubeconfig "${KUBECONFIG_PATH}" stack apply --root "${ROOT}" --concurrency 2 --yes --retry 2

echo ">> resume (noop)"
./bin/torque --kubeconfig "${KUBECONFIG_PATH}" stack apply --root "${ROOT}" --resume --yes --retry 2

echo ">> delete"
./bin/torque --kubeconfig "${KUBECONFIG_PATH}" stack delete --root "${ROOT}" --concurrency 2 --yes --retry 2
